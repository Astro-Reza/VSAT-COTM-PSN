...
class Kalman:
    def __init__(self):
     ...
     def update(self, new_angle, new_rate, dt):
        ...
        self.P[0][0] += dt * (dt*self.P[1][1] - self.P[0][1] - self.P[1][0] + s>
...
# Initialize I2C
bus = smbus.SMBus(3)  # For Orange Pi, usually I2C-1
...
try:
    while True:
        ...
        accPitch = math.atan2(-AccX, math.sqrt(AccY**2 + AccZ**2)) * 180 / math>

        AngleRoll = kalmanRoll.update(accRoll, gyroXrate, dt)
        AnglePitch = kalmanPitch.update(accPitch, gyroYrate, dt)

        print(f"{accRoll:.2f},{accPitch:.2f},{AngleRoll:.2f},{AnglePitch:.2f}")
        time.sleep(0.05)
