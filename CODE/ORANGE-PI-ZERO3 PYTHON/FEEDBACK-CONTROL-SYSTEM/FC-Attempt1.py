...
# --- SPI Setup ---
spi = spidev.SpiDev()
spi.open(1, 1)  # Bus 0, Device 0 (check your board setup)
spi.max_speed_hz = 50000

# --- Control Parameters ---
TARGET_PITCH = 0.0  # Target angle in degrees
KP = 1.0            # Proportional gain
TOLERANCE = 1.5     # Degrees
COMMAND_GAP = 0.3   # Seconds between each SPI command
...
# --- Error Calculation ---
def read_error_elevation(angle_pitch):
    return TARGET_PITCH - angle_pitch

def read_tolerance(error):
    return abs(error) > TOLERANCE

def correction_to(error):
    return KP * error

def send_SPI(motor_id, degrees):
    message = f"{motor_id} {int(degrees)}"
    spi.xfer2([ord(c) for c in message])
    print(f"SPI Sent: {message}")
...
try:
    while True:
        ...
        # === PID Control + SPI Messaging Logic ===
        if now - last_command_time >= COMMAND_GAP:
            error = read_error_elevation(AnglePitch)
            if read_tolerance(error):
                correction = correction_to(error)
                send_SPI(1, correction)
