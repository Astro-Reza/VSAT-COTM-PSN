...
def read_MPU6050():
    ...
    gz = twos_complement(data[8] << 8 | data[9]) / 131.0  # deg/s
    return gz
...
def magnetic_norm(mx, my, mz):
    return sqrt(mx**2 + my**2 + mz**2) / 1000.0  # normalize ~1
...
def main_loop():
    ...
    while True:
        ...
        heading_kalman_predict(dt, radians(gyro_z))  # convert deg/s to rad/s
