...
# ----------------------
latest_data = {'pitch': 0.0, 'roll': 0.0}
...
# PID controller
# ----------------------
def pid_control(setpoint, current, prev_error, integral, kp=3.2, ki=0.05, kd=0.1):
...
def live_main(dt=0.05):
    ...
    pitch_pred = deque(maxlen=200)
    pitch_corr = deque(maxlen=200)
    ...
    labels = ['Platform (Unstable)', 'Stabilized (Measured - PID)', 'Correction (PID)']
    ...
    def update(frame):
        ...
        acc_pitch = latest_data['pitch']
        acc_roll = latest_data['roll']
        ...
        angle_pitch_actual += control_output * dt

        pitch_corr.append(angle_pitch_actual)

        q_rel.append(R.from_euler('yx', [angle_pitch, angle_roll], degrees=True))
        error_pitch = angle_pitch - angle_pitch_actual
        error_roll = angle_roll - angle_roll_actual
        q_level.append(R.from_euler('yx', [error_pitch, error_roll], degrees=True))
        ...
        for i in range(3):
            ...
            for vec, qv, col in zip(rot.T, quivers[i], ['b', 'g', 'r']):
                qv.remove()
                quivers[i][['b', 'g', 'r'].index(col)] = axes3d[i].quiver(0, 0, 0, *vec, color=col, length=0.8)
